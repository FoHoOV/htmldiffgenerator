...
def generate_html_diff_folders(folder1_path: str,
    ...
    for file_path in get_all_file_paths_recursive(folder2_path,
                                                  ignored_paths,
                                                  included_extensions,
                                                  excluded_extensions):
        diff = generate_html_diff_files(get_file_path("", file_path.replace(folder2_path, folder1_path)),
        ...
        new_path_starting_index = file_path.find("extracted-")
        if new_path_starting_index == -1:  # because this can be a not extracted file
            new_path_starting_index = 0
        file_path = file_path[file_path.find('\\', new_path_starting_index):]
        write_to_file(diff, get_file_path("", output_path, absolute=True) + "/" + base_output_folder_path + file_path + ".diff.html")
