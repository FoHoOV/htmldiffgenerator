...
def generate_html_diff_folders(folder1_path: str, folder2_path: str, extensions: list[str],output_path:str, just_context: bool = True, context_lines: int = 5):
    ...
    for file1_path in get_all_file_paths_recursive(folder1_path, extensions):
        diff = generate_html_diff_files(get_file_path("", file1_path),
                                        get_file_path("",
                                                      file1_path.replace(folder1_path[
                                                                         folder1_path.find("/extracted-") + len("/extracted-") + 1:],
                                                                         folder2_path[
                                                                         folder2_path.find("/extracted-") + len("/extracted-") + 1:])),
                                        just_context,
                                        context_lines)
        ...
        write_to_file(diff,
                      get_file_path("", output_path, absolute=True) + "/" + base_output_folder_path + file1_path + ".diff.html")
